...
import { collection, addDoc, query, orderBy, onSnapshot, serverTimestamp, getDocs, setDoc, doc, updateDoc, limit } from 'firebase/firestore';
import { ref, uploadBytesResumable, getDownloadURL } from 'firebase/storage';
import { useAuth } from '../context/AuthContext';
// Importamos tu audio de notificación
import notificacionSound from '../assets/notificaciondeconsulta.wav';
...
const ChatPanel = ({ isOpen, onClose }) => {
  ...
  // Referencia para el sonido de notificación
  const audioNotificacion = useRef(typeof Audio !== "undefined" ? new Audio(notificacionSound) : null);

  // 1. CARGA DE CANALES CON ORDENAMIENTO EN TIEMPO REAL
  useEffect(() => {
    if (!isOpen || !user) return;

    // Escuchamos los canales ordenados por su última actualización
    const qCanales = query(collection(db, "canales"), orderBy("ultimoMensajeAt", "desc"));
    const unsubCanales = onSnapshot(qCanales, (snap) => {
      ...
      // Lógica de notificación sonora y visual
      snap.docChanges().forEach((change) => {
        if (change.type === "modified") {
          const canalActualizado = change.doc.data();
          // Si el mensaje nuevo NO es del usuario actual y NO estamos en ese chat
          if (canalActualizado.ultimoRemitenteId !== user.uid && activeChat.id !== change.doc.id) {
            audioNotificacion.current?.play().catch(e => console.log("Audio play interceptado por navegador"));
            setChatsNoLeidos(prev => ({ ...prev, [change.doc.id]: true }));
          }
        }
      });
    ...
    const fetchUsuarios = async () => {
      ...
      setUsuarios(snap.docs.map(d => ({ id: d.id, ...d.data() })).filter(u => u.id !== user?.uid));
    };
    fetchUsuarios();
  ...
  // 2. CARGA DE MENSAJES DEL CHAT ACTIVO
  useEffect(() => {
    if (!user || !isOpen || !activeChat.id) return;

    // Al entrar al chat, marcamos como leído
    setChatsNoLeidos(prev => ({ ...prev, [activeChat.id]: false }));
  ...
  const usuariosFiltrados = usuarios.filter(u => u.nombre?.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none p-4 sm:p-6">
      ...
      <div className="w-full max-w-[950px] h-[85vh] max-h-[800px] bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex overflow-hidden border border-slate-200 pointer-events-auto transform transition-all animate-in zoom-in-95 duration-200 font-sans">
        
        {/* --- SIDEBAR IZQUIERDO --- */}
        <div className="w-72 sm:w-80 bg-[#f8fafc] border-r border-slate-200 flex flex-col z-10">
          
          {/* Header del Pop-up */}
          <div className="h-16 px-4 bg-slate-900 text-white flex justify-between items-center shrink-0 shadow-md z-20">
          ...
          <div className="p-3 shrink-0 bg-[#f8fafc]">
          ...
          <div className="px-3 pb-2 shrink-0">
            <div className="relative group">
              ...
              <input 
                ...
                className="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all shadow-sm"
              />
            </div>
          </div>
              ...
              <>
                ...
                {canalesFiltrados.map(c => (
                  <button key={c.id} onClick={() => setActiveChat({ id: c.id, nombre: c.nombre, tipo: 'canal' })} 
                    className={`w-full text-left px-3 py-2.5 rounded-xl flex items-center gap-3 transition-all relative ${activeChat.id === c.id ? 'bg-blue-50 border border-blue-100 shadow-sm' : 'hover:bg-slate-200/50 text-slate-700 border border-transparent'}`}>
                    
                    {/* INDICADOR DE MENSAJE NO LEÍDO */}
                    {chatsNoLeidos[c.id] && (
                      <div className="absolute top-2 right-2 w-3 h-3 bg-red-500 rounded-full animate-pulse border-2 border-white shadow-sm"></div>
                    )}
              ...
              <>
                ...
                {usuariosFiltrados.map(u => (
                  <button key={u.id} onClick={() => setActiveChat({ id: u.id, nombre: u.nombre, tipo: 'privado', rol: u.rol, isOnline: u.isOnline })} 
                    className={`w-full text-left px-3 py-2 rounded-xl flex items-center gap-3 transition-all ${activeChat.id === u.id ? 'bg-blue-50 border border-blue-100 shadow-sm' : 'hover:bg-slate-200/50 border border-transparent'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-slate-500 truncate capitalize">{u.rol?.replace('_', ' ') || 'Staff Médico'}</p>
                    </div>
                  </button>
                ))}
              </>
        ...
        <div className="flex-1 flex flex-col bg-slate-50/50 relative">
          ...
          <div className="h-16 px-6 bg-white border-b border-slate-200 flex justify-between items-center shrink-0 z-10 shadow-sm">
          ...
          <div className="flex-1 p-6 overflow-y-auto flex flex-col gap-5 bg-[#eef2f6] custom-scrollbar" ref={chatScrollRef} style={{ backgroundImage: 'radial-gradient(#d1d5db 1px, transparent 1px)', backgroundSize: '24px 24px' }}>
                ...
                return (
                  <div key={msg.id} className={`flex w-full ${esMio ? 'justify-end' : 'justify-start'} ${agrupar ? '-mt-3' : 'mt-2'}`}>
                    ...
                    <div className={`flex flex-col max-w-[70%] ${esMio ? 'items-end' : 'items-start'}`}>
                      ...
                      <div className={`relative group px-4 py-2 text-sm shadow-sm ${
                          esMio ? 'bg-blue-600 text-white rounded-2xl rounded-tr-sm' : 'bg-white border border-slate-200 text-slate-800 rounded-2xl rounded-tl-sm'
                        }`}>
                        ...
                        {msg.tipo === 'documento' && (
                          <a href={msg.archivoUrl} target="_blank" rel="noreferrer" className={`flex items-center gap-3 p-2 rounded-xl border transition-colors ${esMio ? 'bg-blue-700/50 border-blue-500 hover:bg-blue-700' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'}`}>
          ...
          <div className="p-4 bg-white border-t border-slate-200 shrink-0 z-20">
            ...
            <form onSubmit={enviarMensajeTexto} className="flex items-end bg-slate-50 border border-slate-300 rounded-2xl p-2 shadow-sm focus-within:bg-white focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-100 transition-all">
              {isRecording ? (
                 ...
                 <>
                    ...
                    <div className="flex pb-1 px-2">
                      {nuevoMensaje.trim() ? (
                         <button type="submit" disabled={uploading} className="p-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 disabled:opacity-50 shadow-md transition-transform hover:scale-105 active:scale-95">
                           <Send size={18} />
                         </button>
                      ) : (
