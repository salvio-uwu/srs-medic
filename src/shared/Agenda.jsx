...
const Agenda = () => {
  ...
  /* ── INVENTARIO (TIEMPO REAL) — solo la sucursal de la sesión ── */
  useEffect(() => {
    const nombreSucursal = sessionSucursal?.nombre || user?.sucursalActual || user?.sucursal || '';
    if (!nombreSucursal) { setAlertasCaducidad([]); return undefined; }
    const qInv = query(collection(db, 'inventario'), where('sucursal', '==', nombreSucursal));
    ...
    return () => unsub();
  }, [sessionSucursal?.nombre, user?.sucursal, user?.sucursalActual]);
  ...
  useEffect(() => {
    ...
    const nextConsultorioId = consultorioActivo.id;
    const nextSucursalId = consultorioActivo.sucursalId || sessionSucursal?.id || '';
    setNuevaCita((prev) => {
      const resolvedSucursalId = nextSucursalId || prev.sucursalId;
      if (prev.consultorioId === nextConsultorioId && prev.sucursalId === resolvedSucursalId) {
        return prev;
      }
      return {
        ...prev,
  ...
  const handleGuardarCita = async (e) => {
    ...
    try {
      ...
      // Abrir pestaña en el mismo gesto del usuario (antes del await) para no perderla
      ...
      // Si es cita de enfermería, redirigir la pestaña ya abierta a la orden de servicio
      ...
      <div className="agenda-root">
        ...
        <header className="app-header">
          ...
          <div className="header-right">
            ...
            <button className="icon-btn" onClick={() => navigate('/pacientes', { state: { from: '/agenda' } })} title="Directorio de pacientes">
              <Users size={16}/>
            </button>
              ...
              <div className="sidebar-right" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div className="panel inv-panel" style={{ flex: '1 1 50%', minHeight: 0, display: 'flex', flexDirection: 'column' }}>
                  <div className="inv-header" style={{ flexShrink: 0 }}>
                    ...
                    <div className="inv-sub">Próximos a caducar · {sessionSucursal?.nombre || user?.sucursalActual || user?.sucursal || 'Sin sucursal'}</div>
                  </div>
