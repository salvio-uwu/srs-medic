...
const CAT_COLORS = {
  ...
  'Guías clínicas': { bg: 'bg-sky-50', border: 'border-sky-200', text: 'text-sky-700' },
...
const CapacitacionMedicos = () => {
  ...
  useEffect(() => {
    const q = query(collection(db, 'catalogo_documentos_capacitacion_medicos'), orderBy('orden', 'asc'));
    ...
    return (
      <div className="h-screen bg-slate-50 flex flex-col text-slate-700 overflow-hidden">
        <header className="bg-white border-b border-slate-200 px-4 sm:px-6 py-2.5 flex items-center gap-3 shrink-0 z-10">
          ...
          <div className="w-9 h-9 bg-gradient-to-br from-sky-500 to-blue-600 rounded-xl flex items-center justify-center text-white shrink-0 shadow-sm">
              ...
              <div className="flex-1 flex items-center justify-center bg-slate-50 p-8">
                <div className="text-center max-w-sm">
                  ...
                  <a href={docSeleccionado.archivoUrl} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-5 py-3 bg-sky-600 hover:bg-sky-700 text-white rounded-xl font-bold text-sm transition-colors shadow-md shadow-sky-500/25">
            ...
            <div className="w-full lg:w-[420px] lg:max-w-[45%] flex flex-col border-l border-slate-200 bg-white shrink-0">
              <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 bg-gradient-to-br from-sky-500 to-blue-600 rounded-lg flex items-center justify-center text-white">
                    <Sparkles size={14} />
                  </div>
              ...
              <div className="px-3 py-2.5 border-b border-slate-100 flex gap-1.5 overflow-x-auto shrink-0 bg-slate-50/50">
                {QUICK_PROMPTS.map((qp) => (
                  <button key={qp.label} onClick={() => { setChatInput(qp.prompt); inputRef.current?.focus(); }}
                    className="shrink-0 px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white text-[10px] font-bold text-slate-600 hover:bg-slate-50 hover:border-sky-200 hover:text-sky-600 transition-colors">
                    {qp.label}
                  </button>
                ))}
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
                {chatMessages.map((msg, idx) => (
                  <div key={idx} className={`flex gap-2.5 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.role === 'assistant' && (
                      <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center text-white shrink-0 mt-0.5">
                        <Bot size={14} />
                      </div>
                    )}
                ...
                {chatLoading && (
                  <div className="flex gap-2.5 justify-start">
                    <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center text-white shrink-0">
                      <Bot size={14} />
                    </div>
              ...
              <div className="shrink-0 border-t border-slate-200 px-3 py-2.5 bg-white">
                <div className="flex items-end gap-2">
                  <textarea
                    ...
                    rows={1}
                    className="flex-1 px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-[13px] font-medium text-slate-800 placeholder:text-slate-400 outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-50 focus:bg-white transition-all resize-none"
                    style={{ minHeight: '42px', maxHeight: '100px' }}
                  />
                  <button
                    onClick={enviarPregunta}
                    disabled={!chatInput.trim() || chatLoading}
                    className="shrink-0 w-10 h-10 bg-sky-600 hover:bg-sky-700 disabled:bg-slate-200 disabled:cursor-not-allowed text-white rounded-xl flex items-center justify-center transition-colors"
                  >
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-700">
      <header className="bg-white border-b border-slate-200 px-4 sm:px-6 py-4 shrink-0">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="w-10 h-10 bg-gradient-to-br from-sky-500 to-blue-600 rounded-xl flex items-center justify-center text-white shadow-md shadow-sky-500/25">
            <BookOpen size={20} />
          </div>
          <div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm font-medium text-slate-700 placeholder:text-slate-400 outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-50 focus:bg-white transition-all"
            />
          </div>
          {categorias.length > 0 && (
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-sm text-slate-400 mt-1 text-center max-w-sm">
              {busqueda || categoriaFiltro
                ...
                : 'El administrador aún no ha cargado documentos de capacitación para médicos.'}
            </p>
          </div>
        ) : (
            ...
            {docsFiltrados.map(doc => {
              ...
              return (
                <button
                  ...
                  className="text-left bg-white rounded-xl border border-slate-200 p-5 hover:border-sky-200 hover:shadow-md hover:shadow-sky-500/5 transition-all group"
                >
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="text-[14px] font-bold text-slate-800 leading-snug group-hover:text-sky-700 transition-colors line-clamp-2">
                        {doc.titulo}
                      </h3>
                      {doc.categoria && (
                  ...
                  <div className="flex items-center gap-3 pt-2 border-t border-slate-100">
                    ...
                    <span className="text-[10px] font-bold text-sky-500 uppercase tracking-wider flex items-center gap-1">
...
export default CapacitacionMedicos;
