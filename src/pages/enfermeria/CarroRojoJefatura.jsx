...
import {
  ...
  AlertTriangle, MapPin, Clock, History,
  TrendingDown, ArrowUpRight, User
} from 'lucide-react';
...
const printSuc = ({ sucursal, matRows, medRows, usuario, fecha }) => {
  ...
  const sec = (t, rows) => { let s = `<tr><td class="sep" colspan="5"></td></tr><tr><td class="st" colspan="5"><b>${t}</b></td></tr><tr><td class="h">No.</td><td class="h" style="text-align:left">Artículo</td><td class="h">A existir</td><td class="h">Existente</td><td class="h">Caducidad</td></tr>`; rows.forEach((r,i) => { s += `<tr><td class="c n">${i+1}</td><td class="c a">${r.articulo}</td><td class="c v">${r.cantidadExistir||''}</td><td class="c v">${r.cantidadExistente||''}</td><td class="c v">${r.caducidad?r.caducidad.split('-').reverse().join('/'):''}</td></tr>`; }); return s; };
...
const CarroRojoJefatura = () => {
  ...
  const data = useMemo(() => {
    const list = sucursales.map(s => {
      ...
      const d = sucData[n];
      // Vigente si es del mes actual (completo o parcial con mesActivo), o legacy parcial sin mes
      const esPeriodoActual = !!d && (
        d.mesActivo === mesActual
        || d.mesBloqueado === mesActual
        || (!d.mesActivo && !d.mesBloqueado && !!(d.materialData || d.medicamentoData))
      );
      const esMesActual = !!d && (d.mesBloqueado === mesActual || d.mesActivo === mesActual);
      const datosVigentes = esPeriodoActual ? d : null;
      ...
      // Completo solo si el mes quedó marcado como bloqueado y todos los ítems tienen existente
      const plantillaLista = pMat.length > 0 || pMed.length > 0;
      const completo = plantillaLista && d?.mesBloqueado === mesActual && (
  ...
  if (loading && !sucursales.length) return (
    <div className="flex flex-col items-center justify-center py-24 gap-3">
      ...
      <p className="text-[13px] text-slate-500 font-medium">Cargando carro rojo...</p>
    </div>
  );

  const statusCfg = {
    ok:        { bg: 'bg-emerald-500', text: 'text-emerald-700', soft: 'bg-emerald-50 border-emerald-200' },
    atencion:  { bg: 'bg-amber-500',   text: 'text-amber-700',   soft: 'bg-amber-50 border-amber-200' },
    critico:   { bg: 'bg-rose-500',    text: 'text-rose-700',    soft: 'bg-rose-50 border-rose-200' },
    sin_datos: { bg: 'bg-slate-400',   text: 'text-slate-500',   soft: 'bg-slate-50 border-slate-200' },
  };

  const stockCell = (pct) => {
    if (pct === null) return 'bg-slate-50 text-slate-400 border-slate-200';
    if (pct < 0.5) return 'bg-rose-50 text-rose-700 border-rose-200';
    if (pct < 0.8) return 'bg-amber-50 text-amber-700 border-amber-200';
    return 'bg-emerald-50 text-emerald-700 border-emerald-200';
  };

  const cadCell = (dd) => {
    if (dd === null) return 'bg-slate-50 text-slate-400 border-slate-200';
    if (dd <= 0) return 'bg-rose-50 text-rose-700 border-rose-200';
    ...
    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-200 bg-slate-50 flex items-center gap-2">
        ...
        <div className="min-w-0">
          <p className="text-[12px] font-bold text-slate-900 uppercase tracking-[0.06em] truncate">{title}</p>
        ...
        <table className="min-w-[780px] w-full text-sm border-collapse">
          <thead>
            <tr className="bg-white border-b border-slate-200">
              ...
              <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Artículo</th>
              <th className="px-3 py-3 text-center text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400 w-[14%]">A existir</th>
              <th className="px-3 py-3 text-center text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400 w-[14%]">Existente</th>
              <th className="px-3 py-3 text-center text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400 w-[18%]">Caducidad</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, idx) => {
              ...
              return (
                <tr key={row.id} className={`border-b border-slate-50 ${idx % 2 === 1 ? 'bg-slate-50/40' : 'bg-white'} hover:bg-slate-50`}>
                  ...
                  <td className="px-3 py-2.5 text-center">
                    <span className="inline-flex items-center justify-center min-w-16 px-2.5 py-1.5 rounded-md border bg-slate-50 border-slate-200 text-slate-700 text-[12px] font-bold tabular-nums">
                      {row.cantidadExistir || '—'}
                    </span>
                  </td>
                  <td className="px-3 py-2.5 text-center">
                    <span className={`inline-flex items-center justify-center min-w-16 px-2.5 py-1.5 rounded-md border text-[12px] font-bold tabular-nums ${stockCell(pct)}`}>
                      {row.cantidadExistente || '—'}
                    </span>
                  </td>
                  <td className="px-3 py-2.5 text-center">
                    <span className={`inline-flex items-center justify-center min-w-24 px-2.5 py-1.5 rounded-md border text-[11px] font-bold tabular-nums ${cadCell(dd)}`}>
          ...
          return (
            <div key={row.id} className="px-4 py-3">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-[12px] font-semibold text-slate-800 leading-snug truncate">{row.articulo}</p>
                  <p className="text-[10px] text-slate-400 font-semibold mt-0.5">#{idx + 1}</p>
                </div>
                <span className={`shrink-0 px-2 py-1 rounded-md border text-[10px] font-bold tabular-nums ${cadCell(dd)}`}>
                  {fmtCad(row.caducidad)}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                <div className="rounded-md border border-slate-200 bg-slate-50 p-2">
                  <p className="text-[9px] uppercase tracking-[0.08em] text-slate-400 font-bold">A existir</p>
                  <p className="text-[13px] font-bold text-slate-800 tabular-nums">{row.cantidadExistir || '—'}</p>
                </div>
                <div className={`rounded-md border p-2 ${stockCell(pct)}`}>
                  <p className="text-[9px] uppercase tracking-[0.08em] font-bold opacity-70">Existente</p>
                  <p className="text-[13px] font-bold tabular-nums">{row.cantidadExistente || '—'}</p>
                </div>
              </div>
            </div>
          );
  ...
  const filterChips = [
    { key: 'all',       label: 'Total',     count: data.list.length },
    { key: 'ok',        label: 'OK',        count: data.st.ok },
    { key: 'atencion',  label: 'Atención',  count: data.st.atencion },
    { key: 'critico',   label: 'Críticas',  count: data.st.critico },
    { key: 'sin_datos', label: 'Sin datos', count: data.st.sinDatos },
  ];
    ...
    <div className="space-y-4 text-slate-700">
      <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {filterChips.map(sc => {
            ...
            return (
              <button
                key={sc.key}
                type="button"
                onClick={() => setFilterStatus(active && sc.key !== 'all' ? 'all' : sc.key)}
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-900 text-white rounded-md text-[11px] font-semibold hover:bg-slate-800 transition-colors"
        >
      ...
      <div className="space-y-2">
        {filteredList.length === 0 && (
          <div className="text-center py-16 bg-white border border-slate-200 rounded-lg">
            <MapPin size={22} className="text-slate-300 mx-auto mb-3" />
            <p className="text-sm font-semibold text-slate-500">Sin sucursales en este estado</p>
            <button type="button" onClick={() => setFilterStatus('all')} className="mt-2 text-[12px] font-semibold text-slate-900 hover:underline">
              Ver todas
            </button>
          </div>
        )}
        ...
        {filteredList.map(suc => {
          ...
          return (
            <div
              key={suc.nombre}
              className={`rounded-lg border overflow-hidden transition-colors bg-white ${
                open ? 'border-slate-300' : 'border-slate-200 hover:border-slate-300'
              }`}
            >
              <div
                className="flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3.5 cursor-pointer select-none"
                onClick={() => setOpenSuc(open ? null : suc.nombre)}
              >
                <div className="relative shrink-0">
                  <ProgressRing pct={suc.coverage} size={42} stroke={3} color={covColor} />
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-[10px] font-bold text-slate-700 tabular-nums">{suc.coverage}<span className="text-[7px] text-slate-400">%</span></span>
                  </div>
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full shrink-0 ${sc.bg}`} />
                    <h3 className="font-semibold text-[14px] sm:text-[15px] text-slate-900 truncate leading-tight">{suc.nombre}</h3>
                  </div>
                  <div className="flex items-center gap-2 mt-1 text-[11px] text-slate-500 font-medium">
                    {suc.por ? (
                      <span className="truncate max-w-[140px] flex items-center gap-1"><User size={10} className="shrink-0" />{suc.por}</span>
                    ) : (
                    ...
                    {suc.esMesActual && suc.completo && (
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-md border hidden sm:inline-block ${sc.soft} ${sc.text}`}>Completado</span>
                    )}
                    {suc.esMesActual && !suc.completo && (
                      <span className="text-[9px] font-bold text-slate-600 bg-slate-50 border border-slate-200 px-1.5 py-0.5 rounded-md hidden sm:inline-block">En progreso</span>
                    )}
                    {suc.ultimoMesBloqueado && !suc.esMesActual && (
                      <span className="text-[9px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-md hidden sm:inline-block">Pendiente</span>
                    )}
                  </div>
                </div>
                ...
                <div className="hidden sm:flex items-center gap-1.5 shrink-0">
                  {suc.cVenc > 0 && (
                    <span className="inline-flex items-center gap-1 text-[10px] font-bold text-rose-700 bg-rose-50 border border-rose-200 px-2 py-1 rounded-md">
                ...
                <span className="text-[10px] text-slate-400 shrink-0 hidden lg:block font-medium tabular-nums">{fmtTs(suc.ult)}</span>

                <div className="flex items-center gap-0.5 shrink-0">
                  <button
                    type="button"
                    onClick={e => { e.stopPropagation(); printSuc({ sucursal: suc.nombre, matRows: suc.mat, medRows: suc.med, usuario: user?.nombre||'' }); }}
                    className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-50 rounded-md transition-colors" title="Imprimir"
                  >
                    <Printer size={15} />
                  </button>
              ...
              {!open && (suc.cVenc > 0 || suc.cProx > 0 || suc.sCrit > 0 || suc.esMesActual || suc.ultimoMesBloqueado) && (
                <div className="sm:hidden flex items-center gap-1.5 px-4 pb-3 -mt-1 overflow-x-auto">
                  {suc.esMesActual && suc.completo && (
                    <span className="text-[9px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md shrink-0">Completado</span>
                  )}
                  {suc.esMesActual && !suc.completo && (
                    <span className="text-[9px] font-bold text-slate-600 bg-slate-50 border border-slate-200 px-2 py-0.5 rounded-md shrink-0">En progreso</span>
                  )}
                  {suc.ultimoMesBloqueado && !suc.esMesActual && (
                    <span className="text-[9px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-md shrink-0">Pendiente</span>
                  )}
                  {suc.cVenc > 0 && <span className="text-[9px] font-bold text-rose-700 bg-rose-50 border border-rose-200 px-2 py-0.5 rounded-md shrink-0">{suc.cVenc} vencidos</span>}
                  {suc.cProx > 0 && <span className="text-[9px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-md shrink-0">{suc.cProx} próximos</span>}
                  {suc.sCrit > 0 && <span className="text-[9px] font-bold text-rose-700 bg-rose-50 border border-rose-200 px-2 py-0.5 rounded-md shrink-0">↓{suc.sCrit} stock</span>}
                </div>
              )}

              {open && (
                <div className="border-t border-slate-200">
                  <div className="px-4 sm:px-5 py-3 bg-slate-50 flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-3 text-[11px] text-slate-500 font-medium flex-wrap">
                      ...
                      {suc.por && <span>por <b className="text-slate-700">{suc.por}</b></span>}
                      <span className="hidden sm:inline text-slate-300">·</span>
                      <span className="hidden sm:inline"><b className="text-slate-700">{suc.filled}</b> de {suc.all.length} registrados</span>
                    </div>
                    <button
                      type="button"
                      ...
                      {suc.status === 'sin_datos' && (
                        <div className="text-center py-10">
                          ...
                          <button
                            ...
                            className="mt-3 text-[12px] font-semibold text-slate-900 hover:underline flex items-center gap-1 mx-auto"
                          >
                                    ...
                                    <table className="min-w-[760px] w-full text-sm border-collapse">
                                      <thead>
                                        <tr className="bg-slate-50 border-b border-slate-200">
                                          <th className="px-3 py-3 text-left text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Mes</th>
                                          <th className="px-3 py-3 text-left text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Fecha</th>
                                          <th className="px-3 py-3 text-left text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Usuario</th>
                                          <th className="px-2 py-3 text-center text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Capt.</th>
                                          <th className="px-2 py-3 text-center text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">OK</th>
                                          <th className="px-2 py-3 text-center text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Bajo</th>
                                          <th className="px-2 py-3 text-center text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Crit.</th>
                                          <th className="px-2 py-3 text-center text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Venc.</th>
                                          <th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400">Acciones</th>
                                        </tr>
                                      </thead>
